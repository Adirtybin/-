...
public class Robot extends TimedRobot {
  ...
  private NetworkTableEntry CurrentShooterSpeed_unitPer100ms = NewTab.add("CurrentShooterSpeed_unitPer100ms", 0)
      .getEntry();
  ...
  public double color_select(String team, String color) {
    double color_spin = 0;
    if (team == "red" & color == "blue") {
        color_spin = 20;
    } else if (team == "blue" & color == "red") {
        color_spin = 20;
    } else {
      color_spin = 0;
    }
  ...
  public void kaipao() {
    if (frontcam.getLatestResult().hasTargets()) {
      if ((degtoenc(tarpitch) - encoder_pitch.getPosition()) < 5) {
        ...
        SmartDashboard.putBoolean("velo???",
  ...
  // TODO:自动阶段判断球是否发射出去
  public double ball_count_auto(String team, String color) {
    double color_spin = 0;
    Boolean ballCount = m_colorsensor.isConnected();
    if (team == "red" & color == "blue") {
        color_spin = 20;
    } else if (team == "blue" & color == "red") {
        color_spin = 20;
